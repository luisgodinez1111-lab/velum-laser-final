...
export const syncAppointmentWorkflow = async () => {
  ...
  const autoConfirmed = await withTenantContext(async (tx) => tx.appointment.updateMany({
    where: {
      status: "scheduled",
      startAt: {
  ...
  const overdueAll = await withTenantContext(async (tx) => tx.appointment.findMany({
    where: {
