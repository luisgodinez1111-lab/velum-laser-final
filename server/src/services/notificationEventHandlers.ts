...
export const onCustomChargeCreated = async (params: {
  ...
  sendNotificationEmail(params.userEmail, {
    ...
    body: `El equipo de Velum Laser ha generado un cobro de <strong>${params.amountFormatted}</strong> por concepto de <strong>${esc(params.chargeTitle)}</strong>.`,
...
export const onCustomChargePaid = async (params: {
  ...
  sendNotificationEmail(params.userEmail, {
    ...
    title: "¡Pago confirmado!",
    body: `Tu pago de <strong>${params.amountFormatted}</strong> por concepto de <strong>${esc(params.chargeTitle)}</strong> fue procesado exitosamente. El equipo de Velum Laser ya recibió la confirmación.`,
  ...
  sendAdminNotificationEmail({
    ...
    body: `<strong>${esc(params.userName)}</strong> (${esc(params.userEmail)}) ha pagado el cobro personalizado <strong>${esc(params.chargeTitle)}</strong> por <strong>${params.amountFormatted}</strong>.`,
...
export const onAppointmentDepositPaid = async (params: {
  ...
  sendAdminNotificationEmail({
    ...
    body: `<strong>${esc(params.userName)}</strong> (${esc(params.userEmail)}) pagó su depósito para una cita el <strong>${dateLabel}</strong>.`,
...
export const onAppointmentBooked = async (params: {
  ...
  sendAdminNotificationEmail({
    ...
    body: `<strong>${esc(params.userName)}</strong> (${esc(params.userEmail)}) agendó una cita para el <strong>${params.date}</strong> a las <strong>${params.time}</strong>${params.treatment ? ` · ${esc(params.treatment)}` : ""}${params.cabin ? ` · Cabina ${esc(params.cabin)}` : ""}.<br><br><a href="${directConfirmUrl}" style="display:inline-block;background:#1a1614;color:#fff;padding:12px 24px;border-radius:8px;text-decoration:none;font-weight:bold;">Confirmar cita (1 clic)</a>&nbsp;&nbsp;<a href="${confirmUrl}" style="color:#544538;">Ver en agenda</a>`,
...
export const onAppointmentCancelledByPatient = async (params: {
  ...
  sendAdminNotificationEmail({
    ...
    body: `<strong>${esc(params.userName)}</strong> (${esc(params.userEmail)}) canceló su cita del <strong>${params.date}</strong> a las <strong>${params.time}</strong>${params.treatment ? ` · ${esc(params.treatment)}` : ""}${params.reason ? `.<br>Motivo: ${esc(params.reason)}` : "."}`,
...
export const onNewMember = async (params: {
  ...
  sendAdminNotificationEmail({
    ...
    body: `<strong>${esc(params.userName)}</strong> (${esc(params.userEmail)}) acaba de registrarse en la plataforma de Velum Laser.`,
...
export const onMembershipPaymentFailed = async (params: {
  ...
  sendNotificationEmail(params.userEmail, {
    ...
    body: `Hola <strong>${esc(params.userName)}</strong>, no pudimos procesar tu pago de <strong>${params.amountFormatted}</strong>${params.planCode ? ` del plan <strong>${esc(params.planCode)}</strong>` : ""}. Para continuar disfrutando de tus beneficios, por favor actualiza tu método de pago en Stripe.`,
  ...
  sendAdminNotificationEmail({
    ...
    title: "Pago de membresía fallido",
    body: `<strong>${esc(params.userName)}</strong> (${esc(params.userEmail)}) tuvo un cobro fallido de <strong>${params.amountFormatted}</strong>${params.planCode ? ` en el plan <strong>${esc(params.planCode)}</strong>` : ""}. Su membresía pasó a estado "Pago vencido".`,
...
export const onMembershipActivated = async (params: {
  ...
  sendAdminNotificationEmail({
    subject: adminTitle,
    title: adminTitle,
    body: `<strong>${esc(params.userName)}</strong> (${esc(params.userEmail)}) ${params.isRenewal ? "renovó" : "activó"} su membresía${params.planCode ? ` con el plan <strong>${esc(params.planCode)}</strong>` : ""}.`,
...
export const onIntakeApproved = async (params: {
  ...
  sendNotificationEmail(params.userEmail, {
    ...
    body: `Hola <strong>${esc(params.userName)}</strong>, tu expediente médico fue revisado y aprobado por nuestro equipo clínico. Ya puedes agendar tu primera cita.`,
...
export const onIntakeRejected = async (params: {
  ...
  sendNotificationEmail(params.userEmail, {
    ...
    body: `Hola <strong>${esc(params.userName)}</strong>, tu expediente médico fue revisado y requiere algunas correcciones antes de continuar.${params.rejectionReason ? `<br><br><strong>Motivo:</strong> ${esc(params.rejectionReason)}` : ""}<br><br>Por favor actualiza tu expediente para continuar.`,
